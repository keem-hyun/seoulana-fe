	...
	return (
		<div className="flex flex-col min-h-screen bg-gradient-to-b from-pink-light/40 to-purple-light/30">
			...
			<div className="absolute inset-0 -z-10 overflow-hidden">
				...
				<div
					className="absolute bottom-[20%] right-[15%] w-52 h-52 rounded-full bg-purple-light/60 blur-2xl animate-float"
					style={{ animationDelay: '2s' }}
				></div>
				<div
					className="absolute top-[45%] right-[10%] w-32 h-32 rounded-full bg-pink-medium/40 blur-2xl animate-float"
			...
			<main className="flex-grow p-6 md:p-10 max-w-5xl mx-auto w-full">
				...
				>
							...
							<div className="flex flex-col items-center">
								<div className="relative group">
									<div className="w-48 h-48 rounded-full border-4 border-pink-light overflow-hidden bg-pink-light/20 flex items-center justify-center relative">
										{previewImage ? (
											<Image src={previewImage} alt="Profile preview" fill style={{ objectFit: 'cover' }} />
										) : (
											<Image
												src={profileImage}
												alt="Profile"
									...
									/>
									<div
										className="absolute -bottom-2 right-2 w-12 h-12 rounded-full bg-pink-primary shadow-pink flex items-center justify-center cursor-pointer hover:bg-pink-dark transition-colors"
										onClick={triggerFileInput}
									>
										<svg
								...
								<p className="text-sm text-gray-500 mt-4">Upload a profile picture (max 5MB)</p>
							...
							<div className="flex-1">
								<div className="grid gap-6">
									...
									<div>
										...
										<p className="text-sm text-gray-500 mt-1">Connect your wallet to update</p>
									</div>
								</div>
							</div>
						</div>
					...
					<div className="bg-white rounded-3xl shadow-soft border-2 border-pink-light p-8 mb-8">
						<h3 className="text-2xl font-bold text-purple-dark mb-6">Connect Social Accounts</h3>
							...
							<div className="flex items-center justify-between p-4 border-2 border-pink-light rounded-2xl hover:border-pink-primary transition-colors">
								<div className="flex items-center gap-4">
									...
									<div>
										...
										<p className="text-gray-500 text-sm">{twitterConnected ? 'Connected' : 'Not connected'}</p>
									</div>
								</div>
								<button
							...
							<div className="flex items-center justify-between p-4 border-2 border-pink-light rounded-2xl hover:border-pink-primary transition-colors">
								<div className="flex items-center gap-4">
									...
									<div>
										...
										<p className="text-gray-500 text-sm">{telegramConnected ? 'Connected' : 'Not connected'}</p>
									</div>
								</div>
								<button
									className={`px-5 py-2 rounded-full font-medium transition-all ${
			...
			<footer className="w-full bg-gradient-to-r from-pink-primary to-purple-primary py-6 mt-auto">
				<div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center">
					...
					<div className="flex space-x-6 text-sm">
						<a href="#" className="text-white hover:text-purple-light transition-colors">
							Terms
						</a>
						<a href="#" className="text-white hover:text-purple-light transition-colors">
							Privacy
